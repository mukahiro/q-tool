    ...
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-600">
          まだルームがありません。授業を始める前に新しいルームを作成しましょう。
        </p>
      </section>
    );
  }
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  作成日: {dateFormatter.format(new Date(room.createdAt))}
                </p>
              </div>
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left sm:text-right">
                ...
                <p className="font-mono text-lg font-bold tracking-wide text-slate-950">
                  {room.inviteCode}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="break-all text-sm text-slate-600">
                参加URL: /room/{room.inviteCode}
              </p>
              <a
                href={`/room/${room.inviteCode}`}
                className="inline-flex min-h-10 items-center justify-center rounded-md bg-slate-950 px-4 text-sm font-semibold text-white transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
              >
                ルームを開く
              </a>
            </div>
